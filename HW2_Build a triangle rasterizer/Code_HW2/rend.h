...
public:
	...
	GzCoord current;
	Edge() {};
	void assign(const GzCoord& s, const GzCoord& e) {
		for (int i = 0; i < 3; i++) {
			start[i] = s[i];
			end[i] = e[i];
		}
	}
	float slopeX(const GzCoord& s, const GzCoord& e) {
		return (e[0] - s[0]) / (e[1] - s[1]);
	}
	float slopeZ(const GzCoord& s, const GzCoord& e) {
		return (e[2] - s[2]) / (e[1] - s[1]);
	}
...
public:
	...
	float slopeZ(const GzCoord& start, const GzCoord& end) {
		return (end[2] - start[2]) / (end[0] - start[0]);
	}
};
#endif // !SPAN
