...
int Application5::Initialize()
{
	...
	GzToken     nameListOFF[2];
	GzPointer   valueListOFF[2];
	...
	final_render = new GzRender(m_nWidth, m_nHeight);
	final_render->GzDefault();
	m_pFrameBuffer = final_render->framebuffer;
    ...
    for(int i = 0; i < AAKERNEL_SIZE; i++){
        ...
        //set up offsets for each render
        nameListOFF[0]  = GZ_AASHIFTX;
        valueListOFF[0] = (GzPointer)&(AAFilter[i][X]);
        nameListOFF[1]  = GZ_AASHIFTY;
        valueListOFF[1] = (GzPointer)&(AAFilter[i][Y]);
        
        status |= m_pRender[i]->GzPutAttribute(2, nameListOFF, valueListOFF);
...
int Application5::Render() 
{
	...
	//computer weighted sum of all FBS
	for (int i = 0; i < final_render->xres * final_render->yres; i++) {
		GzPixel pixel;
		pixel.red = pixel.green = pixel.blue = pixel.z = 0;
		for (int j = 0; j < AAKERNEL_SIZE; j++) {
			pixel.red += m_pRender[j]->pixelbuffer[i].red * AAFilter[j][2];
			pixel.green += m_pRender[j]->pixelbuffer[i].green * AAFilter[j][2];
			pixel.blue += m_pRender[j]->pixelbuffer[i].blue * AAFilter[j][2];
			pixel.z += m_pRender[j]->pixelbuffer[i].z * AAFilter[j][2];

		}
		final_render->pixelbuffer[i] = pixel;
	}

	final_render->GzFlushDisplay2File(outfile); 	/* write out or update display to file*/
	final_render->GzFlushDisplay2FrameBuffer();	// write out or update display to frame buffer
